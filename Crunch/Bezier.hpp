...
struct BezierTraits<stick::Float32>
{
    ...
    //@NOTE: While most operations work fine with Float32, intersections can be inaccurate due to
    //lack of numeric precision. If you encounter that case, consider switching to Float64.
    ...
    constexpr static stick::Float32 fatLineEpsilon = 1e-3f;
...
class BezierCubic
{
  public:
    ...
    constexpr static ValueType trigonometricEpsilon =
        detail::BezierTraits<ValueType>::trigonometricEpsilon;
     ...
     */
    BezierCubic(const VectorType & _pointOne,
                const VectorType & _handleOne,
                const VectorType & _handleTwo,
                const VectorType & _pointTwo);

    /**
     * @brief Returns the position of the bezier curve at the provided parameter.
     * @param _t The parameter value in the range 0 (the beginning of the curve) to 1 (the end of
     * the curve). Note: The sampling is not linear, meaning i.e. that a _t of 0.5 most likely won't
     * give you the middle of the curve.
     */
    VectorType positionAt(ValueType _t) const;

    /**
     * @brief Returns the normalized normal (the direction perpendicular to the curve) at the
     * provided parameter.
     * @param _t The parameter value in the range 0 (the beginning of the curve) to 1 (the end of
     * the curve). Note: The sampling is not linear, meaning i.e. that a _t of 0.5 most likely won't
     * give you the middle of the curve.
     ...
     * the curve). Note: The sampling is not linear, meaning i.e. that a _t of 0.5 most likely won't
     * give you the middle of the curve.
     * @return Returns a pair of two bezier curves, where the first element is the curve from 0 to
     * _t and the second element is the curve from _t to 1.
     */
    Pair subdivide(ValueType _t) const;

    /**
     * @brief Calculates a sub curve slice of this bezier curve.
     * @param _fromT The parameter value in the range 0 (the beginning of the curve) to 1 (the end
     * of the curve). Note: The sampling is not linear, meaning i.e. that a _t of 0.5 most likely
     * won't give you the middle of the curve.
     * @param _toT The parameter value in the range 0 (the beginning of the curve) to 1 (the end of
     * the curve). The to paramater should be bigger than the from parameter. Note: The sampling is
     * not linear, meaning i.e. that a _t of 0.5 most likely won't give you the middle of the curve.
     ...
     * @param _distance Outputs the distance from the provided position to the closest point on the
     * curve.
     * @param _startT Limits the curve to a starting parameter.
     * @param _endT Limits the curve to an ending parameter.
     * @param _targetDistance Allows you to specify a target distance that the found
     * position/parameter should keep compared to the input position.
     * @return The resulting curve parameter.
     */
    ValueType closestParameter(const VectorType & _point,
                               ValueType & _distance,
                               ValueType _startT,
                               ValueType _endT,
                               ValueType _targetDistance) const;
     ...
     */
    ValueType closestParameter(const VectorType & _point,
                               ValueType _startT = 0.0,
                               ValueType _endT = 1.0,
                               ValueType _targetDistance = 0.0) const;
     ...
     */
    VectorType closestPoint(const VectorType & _point,
                            ValueType _startT = 0.0,
                            ValueType _endT = 1.0,
                            ValueType _targetDistance = 0.0) const;
     ...
     * @param _offset A value between 0 (the beginning of the curve) and length() (the end of the
     * curve).
     */
    ...
    SolveResult<ValueType> solveCubic(ValueType _val,
                                      bool _bHorizontal,
                                      ValueType _min = 0,
                                      ValueType _max = 1) const;
    ...
    struct IntersectionResult
    {
        ...
        inline void append(ValueType _parameterOne,
                           ValueType _parameterTwo,
    ...
    SolveResult<ValueType> inflections(ValueType _minParameter = curveTimeEpsilon,
                                       ValueType _maxParameter = 1 - curveTimeEpsilon) const;

    void biarcs(BiarcResultArray & _outArcs, ValueType _tolerance) const;

    static ValueType arcLength(ValueType _t,
                               const VectorType & _a,
                               const VectorType & _b,
                               const VectorType & _c);

  private:
    bool refine(const VectorType & _point,
                ValueType & _minDist,
                ValueType & _minT,
                ValueType _t,
                ValueType _startT,
    ...
    // ValueType integrateLength(ValueType _t0, ValueType _t1, const VectorType & _a, const
    // VectorType & _b, const VectorType & _c) const;
...
template <class T>
BezierCubic<T>::BezierCubic() : m_pointOne(0), m_handleOne(0), m_handleTwo(0), m_pointTwo(0)
{
}

template <class T>
BezierCubic<T>::BezierCubic(const VectorType & _pointOne,
                            const VectorType & _handleOne,
                            const VectorType & _handleTwo,
                            const VectorType & _pointTwo)
    : m_pointOne(_pointOne), m_handleOne(_handleOne), m_handleTwo(_handleTwo), m_pointTwo(_pointTwo)
{
}
...
void setMinMaxForRoots(const SolveResult<T> & _roots,
                       T _tMin,
                       T _tMax,
                       T _padding,
                       T & _inOutMin,
                       ...
                       bool _bX)
{
    for (stick::Int32 i = 0; i < _roots.count; ++i)
    {
        ...
        {
            setMinMax(_bX ? _bezier.positionAt(_roots.values[i]).x
                          : _bezier.positionAt(_roots.values[i]).y,
                      _padding,
                      _inOutMin,
                      _inOutMax);
        }
...
typename BezierCubic<T>::ValueType BezierCubic<T>::area() const
{
    ...
    return (3.0 * m_handleOne.y * m_pointOne.x - 1.5 * m_handleOne.y * m_handleTwo.x -
            ...
            1.5 * m_pointTwo.y * m_handleOne.x + 3.0 * m_pointTwo.y * m_handleTwo.x) /
           10.0;
}

template <class T>
typename BezierCubic<T>::ValueType BezierCubic<T>::closestParameter(const VectorType & _point,
                                                                    ValueType _startT,
                                                                    ValueType _endT,
                                                                    ValueType _targetDistance) const
{
                                                                    ...
                                                                    ValueType _targetDistance) const
{
    ...
    {
        refine(
            _point, minDist, minT, _startT + i * sampleStepSize, _startT, _endT, _targetDistance);
    }
    ...
    {
        if (!refine(
                _point, minDist, minT, minT - sampleStepSize, _startT, _endT, _targetDistance) &&
...
template <class T>
typename BezierCubic<T>::VectorType BezierCubic<T>::closestPoint(const VectorType & _point,
                                                                 ValueType _startT,
                                                                 ValueType _endT,
                                                                 ValueType _targetDistance) const
{
...
typename BezierCubic<T>::ValueType BezierCubic<T>::lengthBetween(ValueType _t0, ValueType _t1) const
{
    ...
    stick::UInt32 iterationCount =
        max(2, min(16, static_cast<stick::Int32>((ceil(abs(_t1 - _t0) * 32)))));
...
template <class T>
bool BezierCubic<T>::refine(const VectorType & _point,
                            ValueType & _minDist,
                            ValueType & _minT,
                            ValueType _t,
                            ValueType _startT,
...
template <class T>
typename BezierCubic<T>::ValueType BezierCubic<T>::arcLength(ValueType _t,
                                                             const VectorType & _a,
                                                             const VectorType & _b,
                                                             const VectorType & _c)
{
    ...
    return sqrt(tmp.x * tmp.x + tmp.y * tmp.y) +
           std::numeric_limits<ValueType>::min(); // to avoid returning 0 (and division by zero)
}
...
// template<class T>
// typename BezierCubic<T>::ValueType BezierCubic<T>::integrateLength(ValueType _t0, ValueType _t1,
// const VectorType & _a, const VectorType & _b, const VectorType & _c) const
// {
...
//         0.9491079123427585245261897}, {0.1834346424956498049394761, 0.5255324099163289858177390,
//         0.7966664774136267395915539, 0.9602898564975362316835609}, {0,
//         0.3242534234038089290385380, 0.6133714327005903973087020, 0.8360311073266357942994298,
//         0.9681602395076260898355762}, {0.1488743389816312108848260, 0.4333953941292471907992659,
//         0.6794095682990244062343274, 0.8650633666889845107320967, 0.9739065285171717200779640},
//         {0, 0.2695431559523449723315320, 0.5190961292068118159257257,
...
//         0.6423493394403402206439846, 0.8015780907333099127942065, 0.9175983992229779652065478,
//         0.9841830547185881494728294}, {0.1080549487073436620662447, 0.3191123689278897604356718,
//         0.5152486363581540919652907, 0.6872929048116854701480198, 0.8272013150697649931897947,
//         0.9284348836635735173363911, 0.9862838086968123388415973}, {0,
//         0.2011940939974345223006283, 0.3941513470775633698972074, 0.5709721726085388475372267,
//         0.7244177313601700474161861, 0.8482065834104272162006483, 0.9372733924007059043077589,
//         0.9879925180204854284895657}, {0.0950125098376374401853193, 0.2816035507792589132304605,
...
//         0.1294849661688696932706114}, {0.3626837833783619829651504, 0.3137066458778872873379622,
//         0.2223810344533744705443560, 0.1012285362903762591525314}, {0.3302393550012597631645251,
//         0.3123470770400028400686304, 0.2606106964029354623187429, 0.1806481606948574040584720,
//         0.0812743883615744119718922}, {0.2955242247147528701738930, 0.2692667193099963550912269,
...
//         0.0404840047653158795200216}, {0.2152638534631577901958764, 0.2051984637212956039659241,
//         0.1855383974779378137417166, 0.1572031671581935345696019, 0.1215185706879031846894148,
//         0.0801580871597602098056333, 0.0351194603317518630318329}, {0.2025782419255612728806202,
//         0.1984314853271115764561183, 0.1861610000155622110268006, 0.1662692058169939335532009,
//         0.1395706779261543144478048, 0.1071592204671719350118695, 0.0703660474881081247092674,
//         0.0307532419961172683546284}, {0.1894506104550684962853967, 0.1826034150449235888667637,
...
//     // TODO: There should be much better educated guesses for
//     // this. Also, what does this depend on? Required precision?
//     stick::UInt32 iterationCount = max(2, min(16, static_cast<stick::Int32>((ceil(abs(_t1 - _t0)
//     * 32)))));
...
bool BezierCubic<T>::isStraight() const
{
    ...
    if (isCollinear(handleOne(), line, geometricEpsilon) &&
        isCollinear(handleTwo(), line, geometricEpsilon))
    {
...
template <class T>
SolveResult<typename BezierCubic<T>::ValueType> BezierCubic<T>::solveCubic(ValueType _val,
                                                                           bool _bHorizontal,
                                                                           ValueType _min,
                                                                           ValueType _max) const
{
    ValueType a, b, c, p1;
    if (_bHorizontal)
    {
...
ClassificationResult<typename BezierCubic<T>::ValueType> BezierCubic<T>::classify() const
{
    ...
    // inflection points.
    ValueType a1 = m_pointOne.x * (m_pointTwo.y - m_handleTwo.y) +
                   m_pointOne.y * (m_handleTwo.x - m_pointTwo.x) + m_pointTwo.x * m_handleTwo.y -
                   m_pointTwo.y * m_handleTwo.x;
    ValueType a2 = m_handleOne.x * (m_pointOne.y - m_pointTwo.y) +
                   m_handleOne.y * (m_pointTwo.x - m_pointOne.x) + m_pointOne.x * m_pointTwo.y -
                   m_pointOne.y * m_pointTwo.x;
    ValueType a3 = m_handleTwo.x * (m_handleOne.y - m_pointOne.y) +
                   m_handleTwo.y * (m_pointOne.x - m_handleOne.x) + m_handleOne.x * m_pointOne.y -
                   m_handleOne.y * m_pointOne.x;
    ValueType d3 = 3 * a3;
    ValueType d2 = d3 - a2;
    ...
    if (isClose(d1, (ValueType)0.0))
    {
        ...
        {
            return detail::classificationHelper<ValueType>(CurveType::Serpentine,
                                                           d3 / (3 * d2)); // 3b.
        }
    }
    ...
    if (d > 0)
    {
        return detail::classificationHelper<ValueType>(
            CurveType::Serpentine, (d2 + f1) / f2, (d2 - f1) / f2); // 1.
    }
    else
    {
        return detail::classificationHelper<ValueType>(
            CurveType::Loop, (d2 + f1) / f2, (d2 - f1) / f2); // 2.
    }
}
...
{
template <class T>
inline void findOverlap(const BezierCubic<T> & _a,
                        const BezierCubic<T> & _b,
                        typename BezierCubic<T>::ValueType _aParameter,
                        bool _bFlip,
                        typename BezierCubic<T>::IntersectionResult & _outResult)
{
    ...
    if (t1 != -1)
    {
        ...
        if (!_outResult.count ||
            (abs(pair[0] - _outResult.values[0].parameterOne) > BezierCubic<T>::curveTimeEpsilon &&
...
template <class T>
typename BezierCubic<T>::IntersectionResult BezierCubic<T>::overlaps(
    const BezierCubic & _other) const
{
    ...
    bool bFlip = lengthSquared(m_pointTwo - m_pointOne) <
                 lengthSquared(_other.m_pointTwo - _other.m_pointOne);
        ...
        line.distance(l2.m_pointTwo) < geometricEpsilon)
    {
        ...
        if (!bStraightBoth && line.distance(l1.m_handleOne) < geometricEpsilon &&
...
struct ConvexHull
{
    struct HullPart
    {
        ...
        HullPart(std::initializer_list<Vector2<T>> _list) : count(0)
        {
...
template <class T>
inline stick::Maybe<T> clipConvexHullPart(const typename ConvexHull<T>::HullPart & _part,
                                          bool _bTop,
                                          T _threshold)
{
...
template <class T>
inline stick::Int32 curveIntersections(const BezierCubic<T> & _a,
                                       const BezierCubic<T> & _b,
                                       typename BezierCubic<T>::IntersectionResult & _outResult,
                                       bool _bFlip,
                                       stick::Int32 _calls,
                                       stick::Int32 _recursion,
                                       typename BezierCubic<T>::ValueType _tMin,
                                       typename BezierCubic<T>::ValueType _tMax,
                                       typename BezierCubic<T>::ValueType _uMin,
                                       typename BezierCubic<T>::ValueType _uMax)
{
    ...
    std::reverse(&reverseHull.bottom.values[0],
                 &reverseHull.bottom.values[0] + reverseHull.bottom.count);
    ...
    else
    {
        ...
        if (*tMaxClip - *tMinClip > 0.8)
        {
            ...
            else
            {
                ...
                _calls = curveIntersections(pair.first,
                                            sliced,
                                            _outResult,
                                            !_bFlip,
                                            _calls,
                                            ...
                                            tMinNew,
                                            tMaxNew);
                _calls = curveIntersections(pair.second,
                                            sliced,
                                            _outResult,
                                            !_bFlip,
                                            _calls,
        ...
        else
        {
            ...
            else
            {
                ...
                _calls = curveIntersections(sliced,
                                            _b,
                                            _outResult,
                                            _bFlip,
                                            _recursion,
...
template <class T>
inline Vector2<T> alignWithLineHelper(const Vector2<T> & _point,
                                      const Vector2<T> & _lineStart,
                                      T _sinVal,
                                      T _cosVal)
{
...
template <class T>
typename BezierCubic<T>::IntersectionResult BezierCubic<T>::intersections(
    const BezierCubic & _other) const
{
    IntersectionResult ret;
    ...
    // printf("HANDE BOUNDS %s %s\n", toString(myHandleBounds.min()).cString(),
    // toString(myHandleBounds.max()).cString()); printf("OTHER HANDE BOUNDS %s %s\n",
    ...
    if (myHandleBounds.overlaps(otherHandleBounds))
    {
        ...
        if (ol.count)
        {
            ...
            {
                ret.append(
                    ol.values[i].parameterOne, ol.values[i].parameterTwo, ol.values[i].position);
            }
        }
        else
        {
            ...
            else if (bStraight1 || bStraight2)
            {
                ...
                else
                {
                    ...
                    VectorType rotatedP1 =
                        detail::alignWithLineHelper(a->m_pointOne, line.positionOne(), s, c);
                    VectorType rotatedH1 =
                        detail::alignWithLineHelper(a->m_handleOne, line.positionOne(), s, c);
                    VectorType rotatedH2 =
                        detail::alignWithLineHelper(a->m_handleTwo, line.positionOne(), s, c);
                    VectorType rotatedP2 =
                        detail::alignWithLineHelper(a->m_pointTwo, line.positionOne(), s, c);
                    BezierCubic bez(rotatedP1, rotatedH1, rotatedH2, rotatedP2);

                    // printf("%s %s %s %s\n", toString(rotatedP1).cString(),
                    // toString(rotatedH1).cString(), toString(rotatedH2).cString(),
                    // toString(rotatedP2).cString());
    ...
    std::sort(&ret.values[0],
              &ret.values[0] + ret.count,
              [](const typename IntersectionResult::Intersection & _a,
                       ...
                       typename BezierCubic<T>::ValueType _tolerance)
{
    ...
    if (ira && irb)
    {
        ...
        if (divergence <= _tolerance)
        {
            ...
            if (cw && asweep < 0)
                asweep = Constants<ValueType>::twoPi() + asweep;
            if (!cw && asweep > 0)
                asweep = asweep - Constants<ValueType>::twoPi();
            if (cw && bsweep < 0)
                bsweep = Constants<ValueType>::twoPi() + bsweep;
            if (!cw && bsweep > 0)
                bsweep = bsweep - Constants<ValueType>::twoPi();
        ...
        else
        {
            ...
            // printf("START %s END %s\n", toString(pair.first.positionTwo()).cString(),
            // toString(pair.second.positionOne()).cString());
...
template <class T>
SolveResult<typename BezierCubic<T>::ValueType> BezierCubic<T>::inflections(
    ValueType _minParameter, ValueType _maxParameter) const
{
...
template <class T>
void BezierCubic<T>::biarcs(BiarcResultArray & _outArcs, ValueType _tolerance) const
{
